...
import { Home, Hammer, Bell, User, BellRing } from "lucide-react";
...
export const FooterNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 w-full bg-gradient-to-r from-emerald-500 to-emerald-600 border-t border-emerald-700 flex justify-around py-2 shadow-xl rounded-t-3xl backdrop-blur-sm z-50" style={{ maxHeight: '65px' }}>
      ...
      <Link to="/notifications-management" className="flex flex-col items-center w-1/5 justify-center">
        <div className={`p-1 ${isActive('/notifications-management') ? 'bg-emerald-700 rounded-full' : ''}`}>
          <BellRing className="w-5 h-5 text-white" />
        </div>
        <span className={`text-xs mt-0.5 ${isActive('/notifications-management') ? 'text-white' : 'text-white/80'}`}>จัดการ</span>
      </Link>
