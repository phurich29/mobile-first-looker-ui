...
export const WatchlistSection = () => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('notification_settings').select('*');
        if (error) {
          throw error;
        }
  ...
  return <>
      ...
      {loading ? <div className="text-center py-4 text-gray-500 bg-white rounded-xl shadow-sm">กำลังโหลดข้อมูล...</div> : watchlistItems.length === 0 ? <div className="text-center py-8 text-gray-500 bg-white rounded-xl shadow-sm">ยังไม่มีรายการติดตาม</div> : <div className="space-y-4">
          {watchlistItems.map(item => <div key={item.id} className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
              <div className="flex justify-between items-start mb-3">
                ...
                <Link to={`/measurement-history/${item.deviceCode}/${item.symbol}?name=${encodeURIComponent(item.name)}`} className="text-xs px-2 py-1 bg-gray-200 text-black rounded-md hover:bg-gray-300 transition-colors">
                  ดูรายละเอียด
                </Link>
              </div>
              
              <div className="text-sm space-y-1">
                {item.type === "max" && <p className="flex justify-between">
                    <span className="text-gray-600">แจ้งเตือน เมื่อสูงกว่า:</span>
                    <span className="font-medium text-red-600">{item.threshold}</span>
                  </p>}
                {item.type === "min" && <p className="flex justify-between">
      ...
      <div className="mt-8">
        ...
        {settingsLoading ? <div className="text-center py-4 text-gray-500 bg-white rounded-xl shadow-sm">กำลังโหลดข้อมูล...</div> : notificationSettings.length === 0 ? <div className="text-center py-8 text-gray-500 bg-white rounded-xl shadow-sm">ยังไม่มีการตั้งค่าแจ้งเตือน</div> : <div className="space-y-4">
            {notificationSettings.map(setting => <div key={setting.id} className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-medium text-emerald-800">
                ...
                <div className="text-sm space-y-1">
                  {setting.max_enabled && <p className="flex justify-between">
                      <span className="text-gray-600">แจ้งเตือน เมื่อสูงกว่า:</span>
                      <span className="font-medium text-red-600">{setting.max_threshold}</span>
                    </p>}
                  {setting.min_enabled && <p className="flex justify-between">
                      <span className="text-gray-600">แจ้งเตือน เมื่อต่ำกว่า:</span>
