-- 🔥 PHASE 2: Update notifications table to include user_id
-- เพิ่ม user_id ใน notifications table ถ้ายังไม่มี

-- Check if user_id column already exists in notifications table
DO $$ 
BEGIN
...
-- Update database function check_notification_thresholds to handle user_id
  ...
  LOOP
    ...
    LOOP
      ...
      IF setting.min_enabled = true AND exceeded_value < setting.min_threshold THEN
        ...
        -- Check if the notification already exists for this condition
        ...
        WHERE device_code = setting.device_code
          ...
          AND user_id = setting.user_id  -- ⭐ เพิ่มการตรวจสอบ user_id
