-- Step 2: Improve get_devices_with_details Function (Fixed)

-- 2.1 Create optimized version with proper error handling
...
BEGIN
  ...
  EXCEPTION
    WHEN query_canceled THEN
      RAISE WARNING 'get_devices_with_details was cancelled/timed out for user %', user_id_param;
      RETURN;
    WHEN OTHERS THEN
      execution_time := clock_timestamp() - start_time;
...
BEGIN
  ...
  EXCEPTION
    WHEN query_canceled THEN
      RAISE WARNING 'get_guest_devices_optimized was cancelled/timed out';
      RETURN;
    WHEN OTHERS THEN
      execution_time := clock_timestamp() - start_time;
