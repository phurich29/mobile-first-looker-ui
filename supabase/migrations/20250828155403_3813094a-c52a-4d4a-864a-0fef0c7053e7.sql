-- สร้างตารางเก็บประวัติการตั้งค่าการแจ้งเตือน
CREATE TABLE public.notification_settings_history (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  device_code TEXT NOT NULL,
  rice_type_id TEXT NOT NULL,
  rice_type_name TEXT NOT NULL,
  action_type TEXT NOT NULL CHECK (action_type IN ('created', 'updated', 'deleted')),
  old_settings JSONB, -- เก็บการตั้งค่าเก่า (สำหรับ update และ delete)
  new_settings JSONB, -- เก็บการตั้งค่าใหม่ (สำหรับ create และ update)
...
-- ผู้ใช้สามารถดูประวัติการตั้งค่าของตัวเองได้
CREATE POLICY "Users can view their own settings history"
ON public.notification_settings_history
FOR SELECT
USING (user_id = auth.uid());

-- Admin สามารถดูประวัติทั้งหมดได้
CREATE POLICY "Admins can view all settings history"
ON public.notification_settings_history
FOR SELECT
USING (is_admin_or_superadmin_safe(auth.uid()));

-- ระบบสามารถบันทึกประวัติได้
CREATE POLICY "System can insert settings history"
ON public.notification_settings_history
FOR INSERT
WITH CHECK (true);

-- เพิ่มคอลัมน์ใน notifications table เพื่อเก็บข้อมูลการตั้งค่าที่เกี่ยวข้อง
ALTER TABLE public.notifications 
ADD COLUMN IF NOT EXISTS settings_snapshot JSONB;

-- เพิ่ม comment อธิบายคอลัมน์ใหม่
COMMENT ON COLUMN public.notifications.settings_snapshot IS 'Snapshot ของการตั้งค่าในขณะที่เกิดการแจ้งเตือน (min_threshold, max_threshold, etc.)';

-- สร้าง function เพื่อบันทึกประวัติการตั้งค่าอัตโนมัติ
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    INSERT INTO public.notification_settings_history (
      user_id, device_code, rice_type_id, rice_type_name,
      action_type, old_settings, new_settings
    ) VALUES (
      NEW.user_id, NEW.device_code, NEW.rice_type_id, NEW.rice_type_name,
      'updated',
      jsonb_build_object(
...
-- สร้าง trigger เพื่อบันทึกประวัติอัตโนมัติ
CREATE TRIGGER notification_settings_audit_trigger
  AFTER INSERT OR UPDATE OR DELETE ON public.notification_settings
  FOR EACH ROW EXECUTE FUNCTION public.log_notification_settings_change();

-- แก้ไข check_notification_thresholds function เพื่อบันทึก settings_snapshot
...
BEGIN
  ...
  LOOP
    ...
    -- สร้าง settings snapshot
    settings_data := jsonb_build_object(
      'min_threshold', setting.min_threshold,
      'max_threshold', setting.max_threshold,
      'min_enabled', setting.min_enabled,
      'max_enabled', setting.max_enabled,
      'rice_type_name', setting.rice_type_name
    );
      ...
      IF setting.min_enabled = true AND exceeded_value < setting.min_threshold THEN
        ...
        IF notification_exists THEN
          ...
          SET notification_count = notification_count + 1,
              ...
              settings_snapshot = settings_data
          WHERE id = notification_id;
          RAISE NOTICE 'Updated existing notification: id=%', notification_id;
        ELSE
          -- Create a new notification with user_id and settings snapshot
          INSERT INTO notifications (
            device_code, rice_type_id, threshold_type, value, 
            notification_message, timestamp, analysis_id, user_id, settings_snapshot
          )
          VALUES (
            setting.device_code, setting.rice_type_id, threshold_type, 
            exceeded_value, message, current_ts, analysis.id, setting.user_id, settings_data
          );
