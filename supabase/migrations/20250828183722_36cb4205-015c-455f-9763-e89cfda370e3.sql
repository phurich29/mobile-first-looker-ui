-- Fix the check_notification_thresholds function
-- Replace MAX(uuid) with proper ORDER BY approach
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF setting.min_enabled = true AND exceeded_value < setting.min_threshold THEN
        ...
        -- FIXED: Use ORDER BY + LIMIT instead of MAX(uuid)
        SELECT COUNT(*) > 0 INTO notification_exists
        FROM notifications
        WHERE device_code = setting.device_code
        ...
        -- Get notification_id using ORDER BY instead of MAX
        SELECT id INTO notification_id
        FROM notifications
        WHERE device_code = setting.device_code
          AND rice_type_id = setting.rice_type_id
          AND threshold_type = threshold_type
          AND user_id = setting.user_id
          AND abs(value - exceeded_value) < 0.001
          AND timestamp > current_ts - INTERVAL '1 day'
  ...
  RAISE NOTICE 'check_notification_thresholds completed: checked % settings, created % notifications', 
    check_count, notification_count;
    
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'check_notification_thresholds failed: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
    -- Don't re-raise the exception to prevent cron job failures
    RETURN;
END;
$function$;